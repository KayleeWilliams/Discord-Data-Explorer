<template>
    <div class="text-background text-center grid grid-cols-5 auto-rows-max gap-4 w-screen mb-8 ">
        ...
        <DoughnutChart class="col-start-2 row-start-4 col-span-2 h-full" :data="data.browsers" title="Top Browsers" />

        <div class="col-start-4 col-span-1 row-start-4 flex flex-col gap-4">
          <single-container title="Total Messages" :value="data.message_analytics.total_messages"/>
          ...
          <single-container title="Sent @everyone" :value="data.message_analytics.everyone_mentions"/>
        </div>
        
        <list-container class="col-start-2 row-start-3 col-span-1" v-show="!showMessageChart" title="Top Friends" :list="data.message_analytics.friends" :toggle="false" />
        <list-container class="col-start-3 row-start-3 col-span-1" v-show="!showMessageChart" title="Top Servers" :list="data.message_analytics.servers" :toggle="false" />
        <list-container class="col-start-4 row-start-3 col-span-1" v-show="!showMessageChart" title="Top Groups" :list="data.message_analytics.groups" :toggle="true" @messageToggle="messageToggle"/>
